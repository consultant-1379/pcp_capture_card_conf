...
if [ "$2" == "" ]; then
	...
	rpmFileLocation=$PWD/target/rpm/ERICpcp_capture_card_conf-CXC1734188/RPMS/x86_64
...
function runMaven {
    mvn -f $PWD/pom.xml -Droot.path=$PWD clean package -Drstate=$rstate 
    rsp=$?
}
...
setRstate
getReason
pkgName="pcp_capture_card_conf_${rstate}.tar.gz"
git clean -df
git checkout $branch
git pull
